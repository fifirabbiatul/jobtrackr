...
export default function Page() {
  ...
  return <WorkspacePage title="Analytics & Insights" eyebrow="Insight dihitung dari data lamaranmu">
    ...
    <div className="grid gap-5 lg:grid-cols-2">
      <section className="rounded-2xl border bg-white p-6 shadow-card"><h3 className="font-bold text-navy">Hiring Funnel</h3><p className="mt-1 text-xs text-slate-400">Click a stage to view matching applications</p>{!total?<Empty/>:<div className="mt-6 space-y-1">{funnelStages.map(stage=>{const count=submitted.filter(application=>application.current_status===stage).length;return <a href={`/applications?status=${encodeURIComponent(stage)}`} key={stage} className="block rounded-xl p-2 transition hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-teal-200" aria-label={`View ${count} applications in ${stage}`}><div className="flex justify-between text-xs"><span>{stage}</span><b>{count}</b></div><div className="mt-1 h-7 rounded bg-slate-100"><div className="h-full rounded bg-gradient-to-r from-navy to-teal" style={{width:`${Math.max(count/total*100,count?8:0)}%`}}/></div></a>})}</div>}</section>
